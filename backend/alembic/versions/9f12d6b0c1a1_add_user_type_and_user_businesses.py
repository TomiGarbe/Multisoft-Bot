"""add user_type and user_businesses
...
user_type_enum = sa.Enum("ADMIN", "BACKDOOR", "BUSINESS_USER", name="user_type_enum")
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("user_id", "business_id", name="uq_user_businesses_user_business"),
    )

    op.execute(
        """
        INSERT INTO user_businesses (user_id, business_id, created_at, updated_at)
    ...
    op.execute(
        """
        WITH user_business_count AS (
          SELECT user_id, COUNT(DISTINCT tenant_id) AS business_count
          FROM tenant_users
        ...
        SET user_type = CASE
            ...
            WHEN COALESCE(ubc.business_count, 0) > 1 THEN 'ADMIN'
            WHEN COALESCE(ubc.business_count, 0) = 1 THEN 'BUSINESS_USER'
            ELSE 'ADMIN'
          END::user_type_enum
        FROM user_business_count ubc
